...
while(FINISHED == False and super_counter < 3):
    ...
    """
    subprocess.run("cp "+str(starter_csv)+" /pscratch/sd/a/asharoff/NEW_DATA_OCT_25_2023/DL4neurons2/"+str(starter_csv), shell=True)
    ...
    the_result1 = ""
    the_result1 = subprocess.run('./M1_all_submit_testing.sh '+str(starter_csv), stdout=subprocess.PIPE, universal_newlines=True, shell=True)
    ...
    subprocess.run("mkdir "+str(trail_name)+str(super_counter), shell=True)
    subprocess.run("cp *.txt "+ str(trail_name)+str(super_counter)+"/", shell=True)
    subprocess.run("cp *.png "+str(trail_name)+str(super_counter)+"/", shell=True)
    subprocess.run("cp *.npy "+ str(trail_name)+str(super_counter)+"/", shell=True)
    subprocess.run("cp *.csv "+ str(trail_name)+str(super_counter)+"/", shell=True)


    subprocess.run("cp /pscratch/sd/a/asharoff/tmp_neuInv/bbp3/L5_TTPC1cADpyr0/"+str(job_id1)+"/out/*.png "+ str(trail_name)+str(super_counter)+"/", shell=True)
